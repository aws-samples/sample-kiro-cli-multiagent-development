...
for op in ops:
    path = op.get('path', '')
    # Read every shape the write tools actually send. `fs_write` and `fs_append` send
    # `text`; `str_replace` sends `oldStr`/`newStr`. Nothing sends `content` — reading
    # only that made this hook a silent no-op from the day it was written (found by
    # security review 2026-07-24, proven by writing `requests>=2.31.0` with no
    # interception). Keep `content` first for the batched `ops` shape.

...
5. **Offer the next step, do not take it.** Recommend whether the debt warrants a hardening spec. If the user agrees, hand off to `/scope` to create a spec whose tasks upgrade the highest-risk shortcuts — do not start implementing here. Harvesting is read-only except for writing the ledger.

## Rules

- Read-only on source code — this prompt only writes/updates `docs/debt.md`.
